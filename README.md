...
API 地址、协议、模型、API Key、增量批量大小、并发数、归核阈值、每片词数、规范锚点数、归核强度和自定义分类规则保存在当前站点的 localStorage：
...
每批只发送 BV 号、标题、作者、描述、现有标签、主分类和备注，不发送封面图片或本地文件。模型必须返回包含 `id`、`tags`、`category`、`topics`、`collections` 和 `reason` 的 JSON；应用只接受当前批次中存在的 BV 号。提示词明确要求每个视频独立判断，不得借用同批其他视频的内容推断或复制标签。
...
| 人工数据保护 | 增量分类只合并标签；全局归核只重命名已有词汇，模型未返回映射的名称原样保留 | 不会因为归核结果缺项而删除标签 |
...
- 默认 20 条，允许设置 5-40 条。
- 已有标签词表按使用次数排序，最多发送 160 个，避免标签库本身无限扩大上下文。
...
“优先复用已有标签”不是只写在分类规则里的口号。增量任务启动时会从完整资料库统计标签及使用次数，形成运行时词表；每个成功波次返回的新标签会立即加入这个词表，再提供给下一波请求。

```text
任务开始：从完整资料库建立标签词表
    ↓
第 1 波请求：携带当前高频标签，最多 160 个
    ↓
本地校验并保存成功结果
    ↓
把新标签加入运行时词表并更新次数
...
| 并发 | 标签复用行为 | 适用场景 |
| ---: | --- | --- |
| 1 | 后一批立即看到前一批新标签，一致性最好 | 首次建立标签体系、重视标签统一 |
| 2 或更高 | 同一波次共享开始时的词表快照，下一波才吸收本波新标签 | API 稳定、希望提高处理速度 |
...
增量任务只能让后续波次参考前面已经完成的标签，无法保证整个运行期间始终使用同一套名称；开启并发后，同一波次的请求更不可能实时交换结果。因此增量分类完成时，任务面板会明确提醒用户再运行一次“全局归核”。

全局归核不重新发送近千条视频，也不重新分类视频。它执行的是全库词汇规范化：
...
- 默认“积极精简”会优先形成更少、更稳定、更可复用的规范标签；也可以切换成均衡或保守模式。
...
- 增量分类会把 AI 标签与现有标签合并并去重。
- 全局归核不会重新判断视频内容，而是把已有标签按全局映射统一命名；主分类、主题和收藏夹保持不变。
- 主分类仅在 AI 返回非空值时更新。
- 主题和建议收藏夹写回对应数组。
- `ai.status`、`ai.model`、`ai.processedAt`、`ai.reason`、`ai.mode` 和 `ai.tagStrategy` 跟随增量结果保存；全局归核写入 `ai.consolidatedAt`、`ai.consolidationModel` 和 `ai.consolidationSummary`。
...
| 并发提高后标签不够统一 | 同一并发波次共享词表快照，不能看到同波其他请求刚生成的标签 | 首次分类使用并发 1；需要速度时再提高，完成后运行全局归核 |
...
| 全局归核后仍有同义标签 | 模型没有为某些名称返回映射、跨片锚点没有覆盖该名称，或认为它们含义不同 | 使用“积极精简”再次运行，或提高每片词数和锚点数量；未映射名称会安全地原样保留 |
| 全局归核请求超过上下文 | 单片来源词、锚点和输出预算仍超过服务限制 | 降低每片词汇数或切片阈值；16K 上下文不建议使用 500+240 的默认组合 |
| 归核把两个标签错误合并 | 模型错误判断了同义关系，积极精简模式风险更高 | 归核前导出 JSON 备份，或改用均衡/保守；本地只保证映射来源合法，不能保证语义判断绝对正确 |
...
- 全局归核按分片顺序执行，不提供暂停，只能等待完成或直接停止；停止时不会应用本次任务的任何映射。
...
全局归核可能包含多个顺序映射请求，但只有全部分片验证通过后才整库保存一次，不会为每个标签或视频单独写盘。

API Key 和 API 配置始终只保存在 `watchlater.ai.config`，不会进入视频数据文件。AI 生成的标签、主分类、主题、建议收藏夹、模型名称、处理时间和分类依据会跟随视频记录进入 JSON、HTML 和 BiliStar 导出；全局归核时间、模型和分片摘要也会跟随保存。流事件数、接收字节和 Token 用量属于当前任务运行状态，不写入视频数据。
